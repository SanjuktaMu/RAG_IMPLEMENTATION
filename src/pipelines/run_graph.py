...
from src.rag_types.graph_rag import get_graph_rag_chain

DATASET_PATH = Path(EVAL_DIR) / "dataset.json"
OUTPUT_PATH = Path(EVAL_DIR) / "results" / "graph_results.json"
LOG_PATH = Path(EVAL_DIR) / "results" / "graph_logs.json"


def parse_args():
	parser = argparse.ArgumentParser(description="Run and evaluate the Graph RAG pipeline")
	parser.add_argument("--limit", type=int, default=None, help="Optional number of dataset rows to process")
	parser.add_argument("--top-k", type=int, default=None, help="Retriever top-k used for initial graph retrieval")
	parser.add_argument("--max-traversal-nodes", type=int, default=10, help="Maximum nodes to traverse in graph walk")
...
def main():
	...
	rag = get_graph_rag_chain(
		db,
		top_k=args.top_k,
		max_traversal_nodes=max(1, args.max_traversal_nodes),
	)
		...
		logs.append(
			{
				"question": question,
				"ground_truth": ground_truth,
				"query_variants": debug_info.get("query_variants", []),
				"start_nodes": debug_info.get("start_nodes", []),
				"traversal_path": debug_info.get("traversal_path", []),
	...
	print("\n✅ Graph RAG results saved successfully!")
	print(f"✅ Graph RAG logs saved successfully: {log_path.as_posix()}")
